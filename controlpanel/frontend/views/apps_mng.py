...
class AppManager:
    """
    This class is to manage some complex process for application entity. This steps for each process here may be
    re-structured into task-oriented pattern. The main intention for this class is to isolate the complicity from
    ...
    def register_app(self, user, app_data):
        ...
        self._create_auth_settigs(new_app, envs, github_api_token, disable_authentication, connections)
    ...
    def trigger_tasks_for_ip_range_removal(self, user, deleted_object):
        affected_app_ids = list(AppIPAllowList.objects
                                .filter(ip_allowlist_id=deleted_object.id)
    ...
    def trigger_tasks_for_ip_range_update(self, user, pre_update_obj, updated_obj):
        if self._has_ip_ranges_changed(pre_update_obj.allowed_ip_ranges, updated_obj.allowed_ip_ranges):
            affected_app_ids = list(AppIPAllowList.objects
                                    .filter(ip_allowlist_id=updated_obj.id)
                                    .values_list("app_id", flat=True)
                                    .distinct())
            for app_id in list(set(affected_app_ids)):
                start_background_task(
                    "app_ip_ranges.update",
    ...
    def _create_auth_settigs(self, app, envs, github_api_token, disable_authentication, connections):
        for env in envs:
