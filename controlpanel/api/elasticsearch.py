...
def bucket_hits_aggregation(bucket_name, num_days=None):
    ...
    q2 = Q("terms", **{"operation.keyword": ["REST.GET.BUCKET", "REST.GET.OBJECT"]})
...
def app_logs(app, num_hours=None):
    ...
    search_obj = (
        Search(using=conn, index=settings.ELASTICSEARCH["indices"]["app-logs"])
        .sort("time_nano")
    ...
    app_filter = Q(
        "term", **{"kubernetes.labels.app.keyword": f"{app.release_name}-webapp"}
    )
    time_filter = Range(
        **{
            "@timestamp": {
