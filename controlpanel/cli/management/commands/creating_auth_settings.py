...
class Command(BaseCommand):
    ...
    def _get_auth0_client_name(self, app_name, env_name):
        return settings.AUTH0_CLIENT_NAME_PATTERN.format(app_name=app_name, env=env_name)
    ...
    def _create_self_defined_secrets(self, env_name, cluster_instance, app_detail):
        ...
        if not params:
            return
        cluster_instance.create_or_update_secrets(env_name=env_name, secret_data=params)
    ...
    def _is_target_repo_ready(self, cluster_instance):
        try:
            ...
            self.stdout.write(
                f"App: {cluster_instance.app['app_name']} has problem with repo,"
                f" error: {ex.__str__()}"
            )
            return False
        return True
            ...
            for deployment_env in app_detail["envs"]:
                try:
                    self._create_auth_settings(
                        deployment_env, cluster_instance, app_detail, app_conf, initial_user_ids
                    )
                    self._create_self_defined_secrets(deployment_env, cluster_instance, app_detail)
                except Exception as ex:
                    self.stdout.write(
                        f"App: {app_item['app_name']} failed to be processed completed, error: {ex.__str__()}"  # noqa
                    )
