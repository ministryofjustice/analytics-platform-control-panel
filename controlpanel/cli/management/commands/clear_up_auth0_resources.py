...
class Command(BaseCommand):
    ...
    def collect_unused_roles(self, auth0_roles, auth0_clients):
        ...
        for cnt, role in enumerate(auth0_roles):
            self._log_info(
                f"{cnt + 1}: start to process {role['applicationId']} - role {role['name']}"
            )
    ...
    def _read_users_by_page(self, auth0_instance, page=0):
        ...
        search_engine = "v3"
        sort = "last_login:1"
        response = auth0_instance.users.list(
            q=query_string, search_engine=search_engine, page=page, sort=sort
        )
