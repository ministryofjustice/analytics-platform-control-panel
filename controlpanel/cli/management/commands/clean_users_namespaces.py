...
class Command(BaseCommand):
    help = (
        "Clean up users' namespace either based on the time that users haven't logged in CP for >= n months, or you "  # noqa: E501
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "--users",
            type=str,
            help="input: The list of users in csv file whose namespace will be cleaned up",  # noqa: E501
        )
        parser.add_argument(
            "-l",
            "--log",
        ...
        parser.add_argument(
            ...
            type=bool,
            default=False,
            help="input: An option for not committing actual action but showing what will be done",  # noqa: E501
        )
    ...
    def _read_user_from_csv(self, user_csv_file):
        ...
        with open(user_csv_file) as csv_file:
            ...
            for row in csv_reader:
                # mock User object to be consistent and also can be used as
                # input when creating a User class
    ...
    def handle(self, *args, **options):
        if not options.get("months") and not options.get("users"):
            raise CommandError(
                "Please specific the months which will be used to query user table or "
