...
@patch("controlpanel.api.tasks.handlers.base.BaseTaskHandler.handle")
def test_cancelled_task_handle_not_run(handle, handler_cls, args):
    completed_task = MagicMock(spec=Task, completed=False, cancelled=True)
    base_task_handler = handler_cls()
...
@patch("controlpanel.api.tasks.handlers.base.BaseTaskHandler.handle")
def test_cancelled_and_completed_task_handle_not_run(handle, handler_cls, args):
    completed_task = MagicMock(spec=Task, completed=True, cancelled=True)
    base_task_handler = handler_cls()
...
def test_uncompleted_task_handle_is_run(handle, handler_cls, args):
    completed_task = MagicMock(spec=Task, completed=False, cancelled=False)
    base_task_handler = handler_cls()
