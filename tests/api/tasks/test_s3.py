...
def test_exception_raised_when_called_without_valid_app(complete, users, task_method, model):
...
def test_bucket_created(cluster, complete, users):
    ...
    cluster.S3Bucket.return_value.create.assert_called_once_with(owner=s3bucket.bucket_owner)
...
def test_access_granted(cluster, complete, users, task_method, model_class, cluster_class):
...
def test_revoke_user_access(cluster, complete, bucket_name, is_folder):
    ...
    if is_folder:
        cluster.User.return_value.revoke_folder_access.assert_called_once_with(bucket_identifier)
    else:
        cluster.User.return_value.revoke_bucket_access.assert_called_once_with(bucket_identifier)
