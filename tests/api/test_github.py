...
def test_get_repository_not_found(request_get_not_found):
    ...
    with pytest.raises(
            RepositoryNotFound, 
            match="Repository 'test-repo-name' not found, it may be private"
        ):
        test_api_token = "abc123"
...
def test_get_repository_contents_success(request_get_success):
    ...
    test_api_token = "abc123"
    response = GithubAPI(test_api_token).get_repository_contents("test-repo-name", "some/resource/path")
