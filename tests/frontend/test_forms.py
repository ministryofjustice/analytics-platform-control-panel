...
def test_quicksight_form_add_to_groups(
    ...
    Super users should not be added to these groups
    """

    test_user = users[user]
    get_user_id.return_value = test_user.identity_center_id

    form = forms.QuicksightAccessForm(user=test_user)
    form.cleaned_data = data
...
def test_quicksight_form_remove_from_group(
    ...
    """
    Tests removing a user from a group. Should still be part of azure holding group
    """
