...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"key": "environment", "value": "prod", "env_name": "test_env"},
            1,
            0,
            {"env_name": "test_env", "key_name": "XXX_environment", "key_value": "prod"},
        ),
        (create, "owner", status.HTTP_200_OK, {}, 0, 0, None),
        (create, "normal_user", status.HTTP_200_OK, {}, 0, 0, None),
        (delete, "superuser", status.HTTP_302_FOUND, {"key": "hello"}, 0, 1, None),
        (delete, "owner", status.HTTP_302_FOUND, {"key": "hello"}, 0, 1, None),
        (delete, "normal_user", status.HTTP_403_FORBIDDEN, {"key": "hello"}, 0, 0, None),
    ],
)
def test_permission(
    client,
