...
def param(users):
    with patch("controlpanel.api.aws.AWSParameterStore.create_parameter"):
        baker.make("api.Parameter", 3, created_by=users["other_user"])
        param = baker.make("api.Parameter", created_by=users["owner"])
        return param
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (list, "external_user", status.HTTP_403_FORBIDDEN),
